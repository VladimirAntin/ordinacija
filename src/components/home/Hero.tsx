  ...
  return (
    <section
      ...
      <div className={'absolute inset-0'}>
        ...
        <div
          className={
            'via-primary-dark/75 absolute inset-0 bg-linear-to-br from-slate-900/90 to-slate-800/60'
          }
        />
      </div>
      ...
      {/* Content */}
      <div
        className={'relative z-10 mx-auto max-w-4xl px-4 pt-24 pb-8 text-center sm:px-6 lg:px-8'}>
        {/* Badge */}
        <div
          className={
            'mb-6 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-4 py-2 backdrop-blur-sm'
          }>
          <span className={'bg-accent-dark h-2 w-2 animate-pulse rounded-full'} />
          <span className={'text-sm font-medium text-white/90'}>
            {t('Stomatološka Ordinacija')}
          </span>
        </div>
        ...
        <div className={'flex flex-col items-center justify-center gap-4 sm:flex-row'}>
          <Link
            href={'/#contact'}
            className={
              'group text-primary hover:bg-accent hover:text-primary-dark flex items-center gap-2 rounded-full bg-white px-8 py-4 font-medium shadow-lg transition'
            }>
            {t('Zakažite Pregled')}
            <ArrowIcon
              height={20}
      ...
      <div
        className={
          'relative z-10 mt-auto w-full border-t border-white/10 bg-slate-900/60 backdrop-blur-md'
        }>
        <div
          className={
            'mx-auto flex max-w-4xl flex-wrap items-center justify-center gap-0 divide-x divide-white/10 px-4'
          }>
